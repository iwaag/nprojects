...
class DnsmasqExportTests(unittest.TestCase):
    ...
    def test_render_outputs_for_ansible_consumption(self) -> None:
        ...
        self.assertEqual(payload["dhcp_ranges"], [])
        self.assertTrue(render_dnsmasq_export_json(export, generated_at="2026-06-23T00:00:00+00:00").endswith("\n"))

    def test_dynamic_desired_ranges_export_stable_dhcp_range_lines(self) -> None:
        active = node("Edge 1", "edge-1", "active")
        export = export_dnsmasq_records(
            [
                endpoint(
        ...
        self.assertEqual(export.summary["dhcp_ranges"], 2)
        self.assertEqual(
            [entry["line"] for entry in export.dhcp_ranges],
            [
        ...
        self.assertIn("dhcp-range=192.168.0.200,192.168.0.210,12h\n", conf)

    def test_desired_range_without_lease_time_omits_fourth_field(self) -> None:
        export = export_dnsmasq_records([], ip_ranges=[ip_range(dnsmasq_options={})])

        self.assertEqual(export.dhcp_ranges[0]["line"], "dhcp-range=192.168.0.200,192.168.0.250")
        self.assertEqual(export.dhcp_ranges[0]["lease_time"], "")

    def test_non_dynamic_or_invalid_ranges_are_skipped(self) -> None:
        export = export_dnsmasq_records(
            [],
            ip_ranges=[
        ...
        self.assertEqual(export.dhcp_ranges, [])
        self.assertEqual(export.summary["skipped_range_details"], 4)
        self.assertEqual(export.summary["skipped_ranges"], 4)
        skipped = {entry["slug"]: entry["reasons"] for entry in export.skipped if entry["item_type"] == "dhcp_range"}
        self.assertEqual(skipped["static"], ["range_policy_not_dhcp_dynamic_pool"])
        self.assertEqual(skipped["off"], ["generate_dnsmasq_false"])
        self.assertEqual(skipped["bad"], ["invalid_start_address"])
        self.assertEqual(skipped["old"], ["range_lifecycle_not_exportable"])

    def test_json_payload_separates_dns_reservations_ranges_and_skipped(self) -> None:
        active = node("Edge 1", "edge-1", "active")
        primary = endpoint(
            name="primary",
            desired_node=active,
        ...
        self.assertEqual(list(payload.keys()), ["schema_version", "generated_at", "job_result_id", "summary", "dns_records", "dhcp_reservations", "dhcp_ranges", "skipped"])
        self.assertEqual(payload["dns_records"][0]["line"], "host-record=edge-1.example.test,192.0.2.10")
        self.assertEqual(payload["dhcp_reservations"][0]["line"], "dhcp-host=aa:bb:cc:dd:ee:ff,edge-1.example.test,192.0.2.10")
        self.assertEqual(payload["dhcp_ranges"][0]["line"], "dhcp-range=192.168.0.200,192.168.0.250,12h")
